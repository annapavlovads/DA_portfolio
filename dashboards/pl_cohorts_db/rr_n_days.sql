...
--покупки в разрезе активных контактов (Retention Rate)
--Анализируется количество активных контактов пользователей в различные временные интервалы после первой покупки.

--Использование:
--Этот запрос помогает оценить, как меняется количество активных контактов в разрезе времени после первой покупки.

--Входные данные:
--Таблица dwh.chequeitems_retro, содержащая информацию о покупках пользователей

--Выходные данные:
--Результатом выполнения запроса является таблица с колонками visit_day_order (порядковый номер дня после первой покупки), 
--"Активных контактов, шт." (количество активных контактов) и 
--"MIN(visit_day_order)" (минимальный порядковый номер дня после первой покупки).
...
--Результатом выполнения данного запроса будет таблица, содержащая данные о количестве активных контактов в различные 
