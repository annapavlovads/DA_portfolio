...
--и выявления особенностей поведения клиентов в момент их первой покупки, а также для анализа характеристик этих покупок.

--Входные данные: 
--Таблица dwh.chequeitems_retro, содержащая информацию о чеках и контактной информации пользователей.

--Выходные данные: 
--Результатом выполнения запроса является таблица со следующими колонками: __timestamp (дата первой покупки в формате ПН), 
--ea_ch (атрибуты чека), "COUNT_DISTINCT(cheq_id)" (количество уникальных чеков).

--Алгоритм:
--1. Выбор данных из таблицы dwh.chequeitems_retro и преобразование временных данных с использованием вспомогательных 
...
--3. Фильтрация данных с использованием предиката оператора WHERE и оставление только первых покупок каждого клиента.
...
--Ожидаемый результат:
--Результатом выполнения запроса будет таблица, содержащая информацию об атрибутах чеков, относящихся к первым покупкам 
--пользователей, и количество уникальных идентификаторов чеков для каждого из них.
