-- SQL-запрос предназначен для анализа использования бонусных баллов участниками программы лояльности 
-- по времени их начисления и списания. 
-- Запрос извлекает данные из таблицы dwh.bonus_slim_retro и рассчитывает коэффициент использования 
-- баллов (отношение списанных и сгоревших баллов к начисленным). 
-- Запрос состоит из нескольких вложенных запросов, включая агрегацию и фильтрацию данных.
-- Полученная информация полезна для анализа паттернов использования бонусных баллов пользователями, 
-- определения эффективности программы лояльности и оценки вовлеченности пользователей в отношении использования бонусов
...
-- дата начала дня
-- коэффициент использования баллов - отношение использованных и списанных баллов к начисленным

-- Алгоритм:
-- 1. Извлечение данных из таблицы dwh.bonus_slim_retro с использованием вложенного подзапроса, 
-- расчет значений начисленных, списанных, сгоревших баллов и остатка баланса
-- 2. Группировка полученных данных по началу месяца (bonus_date)
-- 3. Расчет коэффициента использования баллов на основе полученных значений и фильтрация данных 
-- по временному диапазону
-- 4. Агрегация данных по началу дня и сортировка результатов по коэффициенту использования баллов в порядке убывания

-- Ожидаемый результат:
-- Результатом работы запроса будет таблица, предоставляющая коэффициент использования бонусных баллов для 
-- каждого дня во временном диапазоне
