...
--Выходные данные:
--Результатом выполнения запроса является таблица с колонками: __timestamp (дата начала дня) и "Сальдо, баллов", которая отражает 
--сальдо баллов на начало каждого дня в указанном временном диапазоне.

--Алгоритм:
--1. Извлечение данных из таблицы dwh.bonus_slim_retro с использованием вложенного подзапроса, расчетом начисленных, списанных и 
--сгоревших баллов, и расчетом сальдо баллов на начало каждого месяца.
...
--Ожидаемый результат:
--Результатом работы запроса будет таблица, содержащая сальдо баллов на начало каждого дня в указанном временном диапазоне.
