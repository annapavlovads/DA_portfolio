-- SQL-запрос предназначен для анализа активности пользователей в различные временные интервалы после их первой 
-- покупки (метрика Retention Rate). Чарт динамики метрики, построенный на основе запроса, помогает оценить, 
-- как меняется количество активных контактов в разрезе времени после первой покупки.
...
-- Алгоритм:
-- 1. Создание виртуальной таблицы: для каждого контакта вычисляется когорта (cohort), 
-- размер когорты (cohort_size), максимальный интервал времени после первой покупки (max_diff), 
-- а также количество активных контактов (contacts_qty) в различные временные интервалы после первой покупки
...
-- Результатом выполнения данного запроса является таблица, содержащая данные о количестве активных контактов 
