-- SQL-запрос предназначен для извлечения информации об SMS-коммуникациях с клиентом. 
...
SELECT action_name AS action_name,
       ...
       COUNT(DISTINCT link_visit)/count(distinct contact_id) AS "CTR,%",
       count(distinct link_visit)/count(distinct opened) AS "Click ToOpen Rate,%",
       sum(day_7_cheq)/count(distinct sent) AS "Revenue Per Message, руб.",
       SUM(day_7_cheq)/sum(message_price_rub) AS "ROMI, %"
FROM
  (SELECT contact_id,
          first_name,
          last_name,
          ...
          action_name,
          send_date,
          51800/10000 as message_price_rub,
          case
