--SQL-запрос предназначен для анализа показателей пуш-уведомлений и покупок клиентов. Запрос позволяет оценить 
--общее количество отправленных пуш-уведомлений, уровень доставки уведомлений, конверсию визитов в покупки и 
--общую выручку по различным периодам времени после отправки уведомлений.
--Запрос применяется для анализа эффективности пуш-уведомлений и их влияния на поведение клиентов. 
 
--Входные данные:
--Запрос использует данные из таблиц dwh.communication (содержит информацию о пуш-уведомлениях) и 
--dwh.chequeitems_retro (содержит информацию о покупках и чеках клиентов).
  
--Выходные данные:
--Результатом запроса является таблица с названиями и значениями следующих показателей: общее количество 
--отправленных пуш-уведомлений, уровень доставки уведомлений, конверсия визитов в покупки за 7 и 14 дней 
--после отправки, общая выручка по 7 и 14 дням после отправки.
  
--Алгоритм:
--1. Запрос использует подзапросы для объединения информации о пуш-уведомлениях и покупках клиентов, а также 
--для расчета показателей уровня доставки, конверсии и общей выручки.
